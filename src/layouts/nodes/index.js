...
const SessionCard = ({ item, fetchData, handleNotification }) => {
  ...
  const actionTest = () => {
    ...
    navigate(`/test/${item._id}?color=${color.substring(1)}${item?.isBubbleEnabled ? '&bubbleText=' + item?.bubbleText + '&bubbleColor=' + bubbleColor.substring(1) : ''}&isLeft=${item?.isLeft ? 'true' : 'false'}&isAuto=${item?.isAuto ? 'true' : 'false'}`);
  }
